...
class NotificationService {
  ...
  Future<String> getDeviceToken() async {
    ...
    //   debugPrint('Comet chat fcm Token: $token');
...
class PushNotificationsManager {
  ...
  void _setFCMToken() async {
    ...
    if (settings.authorizationStatus == AuthorizationStatus.authorized) {
      ...
      // if (token != null) {
      //  await CometChatService.registerToken(token, 'apns');
      // }
      initSharePref_devidce(token ?? '');
    } else if (settings.authorizationStatus ==
        AuthorizationStatus.provisional) {
      print("User granted provisional permission");
    } else {
  ...
  Future<void> initSharePref_devidce(
    String device_token,
  ) async {
    prefs = await SharedPreferences.getInstance();
    prefs.setString("device_token", device_token);
  }
}
