...
class SalesforceDataBloc
  ...
  void onGetOpportunitySalesforce(
      GetOpportunitySalesforce event, Emitter<SalesforceDataState> emit) async {
    emit(const SalesforceDataLoading());

    final dataState = await _getSalesForceOpportunity(param: event.userId);
    if (dataState is DataSuccess) {
      emit(SalesforceOpportunityDone(dataState.data!));
    }
